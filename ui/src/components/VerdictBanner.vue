...
<style scoped>
.verdict-banner { padding: 0.6rem 2rem; border-bottom: 1px solid var(--border); }
.verdict-inner { max-width: 1100px; margin: 0 auto; display: flex; align-items: center; gap: 1rem; }
.verdict-label { font-size: 0.8rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; }
...
.verdict-banner.pass { background: rgba(102, 187, 106, 0.12); }
.verdict-banner.pass .verdict-label { color: var(--color-complete); }
.verdict-banner.warn { background: rgba(255, 167, 38, 0.1); }
.verdict-banner.warn .verdict-label { color: var(--color-warning); }
.verdict-banner.fail { background: rgba(239, 83, 80, 0.1); }
